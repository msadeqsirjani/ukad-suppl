...
OUT_PATH = ROOT / "docs" / "paper" / "iclr2027-9page" / "figures" / "ablation.png"
DATASETS = ("busi", "cvc", "glas", "isic")
LABELS = ("BUSI", "CVC", "GlaS", "ISIC")
ARMS = (
    ("ukad_b", "UKAD-B", style.BAR_UKAD),
    ("ukad_no_displacement", "W/O Disp.", style.BAR[0]),
    ("ukad_no_kan", "W/O Rational", style.BAR[1]),
    ("ukad_no_displacement_no_kan", "W/O Disp.+Rational", style.BAR[2]),
    ("ukad_no_context", "W/O Context", style.BAR[3]),
)
...
if __name__ == "__main__":
    style.faceted_bars(collect(), LABELS, "IoU (%)", get_args().out, ncol=5)
